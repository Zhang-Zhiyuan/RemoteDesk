...
远控窗口的 Ctrl+C / Ctrl+X 会尝试取回远端文字，Ctrl+V 会先同步本机文字再粘贴。右键菜单等其他复制方式，可手动发送或取回剪贴板；手机入口在“更多 → 文字剪贴板”。不会在后台无条件同步所有复制内容。
...
被控端的 H.264 硬件编码还需要支持相应编码器的 FFmpeg；没有时会回退到 JPEG。
