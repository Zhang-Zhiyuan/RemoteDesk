...
- The Android host supports negotiated, request-correlated clipboard snapshots for new desktop viewers. Unchanged text is not resent; unavailable/background-restricted reads return an explicit failure and never clear the viewer clipboard. The Android viewer still uses its clipboard menu and keyboard shortcuts; automatic context-menu clipboard sync in the Android viewer is not implemented in this update. Android 10+ clipboard reads generally require the app to have input focus or be the default keyboard ([platform restriction](https://developer.android.com/about/versions/10/privacy/changes#clipboard-data)).
...
- Provides a received-file list in the Android UI. Storage is read in the background; only completed public downloads appear, and read failures are distinguished from an empty folder. Received files can be opened, shared, or inspected in a selectable, scrollable location dialog; app-specific fallback files are exposed through a narrow read-only content provider.
...
- Remote file browsing and pulling arbitrary files from the Android viewer are not yet implemented. The viewer already supports manual text clipboard send/receive/paste and confirmed single-file uploads through both direct and relay connections (with save-location preflight and a final receipt).
